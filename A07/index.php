...
// Fetch groupChatID and senderID
$groupChatID = isset($_GET['groupChatID']) ? $_GET['groupChatID'] : 1; // Default to 1
$senderID = isset($_SESSION['senderID']) ? $_SESSION['senderID'] : 8; // Default to 8 (can change)
...
<body>
        ...
        <div class="header">
            <div class="group-chat-info">
                <div class="group-chat-photo">
                    ...
                    FROM groupChats WHERE groupChatID = $groupChatID";
                    $resultGroupChat = executeQuery($queryGroupChat);

                    if (mysqli_num_rows($resultGroupChat) > 0) {
                        $row = mysqli_fetch_assoc($resultGroupChat);
    ...
    WHERE m.groupChatID = $groupChatID";
            ...
            while ($message = mysqli_fetch_assoc($resultMessages)) {
                ...
                // Edit and Delete Buttons (only for the sender)
                if ($message['senderID'] == $senderID) {
                    echo "<a href='editMessage.php?id=" . $message['messageID'] . "' class='mess-btn'>Edit</a>";
                    echo "<a href='deleteMessage.php?id=" . $message['messageID'] . "&groupChatID=$groupChatID' class='mess-btn' onclick='return confirm(\"Are you sure you want to delete this message?\")'>Delete</a>";
                }
                echo "</div>";
                echo "</div>";
            }
        ...
        <div class="input-section">
            <form action="insertMessage.php" method="POST">
                ...
                <button type="submit" name="sendMessage" class="icon send"
                    style="background-image: url('images/<?php echo htmlspecialchars($icons['sendMessage']); ?>');">
                </button>
            </form>
